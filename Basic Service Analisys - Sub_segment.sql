...
WITH base_data AS (
  ...
  GROUP BY Report_year, Update_at, W, Sub_Segment_Sales
),
cumulative_data AS (
  SELECT 
    *, 
    SUM(growth_basic_all) OVER (
      PARTITION BY Concat(Update_at,Sub_Segment_Sales)
...
END as growth_basic_all,
  ...
  percent_growth
  FROM Realesedata
  WHERE cumulative_growth_basic is not null
  AND cumulative_growth_basic > 0)